...
// Реализуйте следующие методы:
    ...
    Value Evaluate(const SheetInterface& table) const override {
        ...
        try{
             ...
             if(std::isinf(tempval) || std::isnan(tempval)) throw FormulaError(FormulaError::Category::Arithmetic);  
    ...
    std::vector<Position> GetReferencedCells() const override{
         std::vector<Position> pos_s;
          for(auto &&p : ast_.GetCells()){
              if(std::find(pos_s.begin(), pos_s.end(), p) == pos_s.end()) {pos_s.push_back(p);}}
           return pos_s;
    };
