...
std::string Cell::GetText() const {
   if (cell_) {return cell_->GetText();}
   return "";
}

void Cell::PrintDiagnostic() const {
      using namespace std;
      if(cell_ == nullptr) cout<< "I AM EMPTY"<<endl;
      else{ //show diagnostic cellvar
        auto v = GetValue();
        if (holds_alternative<double>(v)) cout<<"I AM DOUBLE:"<<std::get<double>(v)<<endl;
        else if (holds_alternative<std::string>(v)) cout<<"I AM STRING:"<<std::get<std::string>(v)<<endl;
        else {//else 1
            cout<<"I AM ERROR :";
       ...
       if(!from_me_depended_.empty()){//if
        for(auto && i: from_me_depended_){//for
          cout<<i.row<<"->"<<i.col<<endl;
          }//for
        }//if
        else cout<<"NONE"<<endl;
        ...
        cout<<"I DEPEND:";
        if(!depences_.empty()){//if
        for(auto && i: depences_){//for
          cout<<i.row<<"->"<<i.col<<endl;
        }//for
        }//if
        else cout<<"NONE"<<endl;
       ...
       cout<< "MY CACHE: ";
       if(cache_.has_value()){//if has valuse
       if (holds_alternative<double>(cache_.value())) cout<<"I AM DOUBLE:"<<std::get<double>(cache_.value())<<endl;
        else if (holds_alternative<std::string>(cache_.value())) cout<<"I AM STRING:"<<std::get<std::string>(cache_.value())<<endl;
        else {//if ferror
            cout<<"I AM ERROR :";
