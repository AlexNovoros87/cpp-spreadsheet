...
Sheet::~Sheet() {
  for ( auto& row : table_){delete row.second;}
}

void Sheet::SetCell(Position pos, std::string text) {
  if(!pos.IsValid()) throw InvalidPositionException("SetCell");
    ...
    if(HasCircularDepends(pos , temp->GetReferencedCellsLink())){
     ...
     for(auto && p : table_.at(pos)->_GetReferencingCells()){
      ...
      if(print_max_.cols<pos.col) print_max_.cols = pos.col;
      if(print_max_.rows<pos.row) print_max_.rows = pos.row;
      }
    }
...
const Cell* Sheet::GetCell(Position pos) const {
   if(!pos.IsValid()) throw InvalidPositionException("GetCell");
  if(ExistsPosition(pos)) return table_.at(pos);  
  return nullptr;
}
...
void Sheet::ClearCell(Position pos) {
      if(!pos.IsValid()) throw InvalidPositionException("ClearCell");
         ...
         table_.erase(pos);
          ...
          print_max_.rows = -1;
            ...
            for(const auto& i : table_){
             ...
             if(i.first.col >  print_max_.cols) print_max_.cols = i.first.col;
             if(i.first.row > print_max_.rows)  print_max_.rows = i.first.row;}
             }
         }
  }
...
void Sheet::PrintValues(std::ostream& output) const {
    Size limits = GetPrintableSize();
     for(int r = 0; r< limits.rows; ++r){
      ...
      if(std::holds_alternative<std::string>(printable)) output << std::get<0>(printable); 
      else if(std::holds_alternative<double>(printable)) output << std::get<1>(printable);
      else output <<std::get<2>(printable).ToString() <<"!"; }}

      if(c< print_max_.cols)output<<tab;}
      output<<'\n';}      
  }
...
bool Sheet::HasCircularDepends(const Position& pos, const std::vector<Position>& dependes_list) const{ 
  ...
  if(dependes_list.empty())return false; 
   std::set<Position> storage; 
   std::list<Position> queue_to_visit; 
  
    // sheet->SetCell("M6"_pos, "=E2");
     for(const auto& depend : dependes_list){ //for deplist
        queue_to_visit.push_back(depend); // 4 - 1 
        const auto& front = queue_to_visit.front(); 

    while(!queue_to_visit.empty()){//while not empty 
      if(table_.count(front) > 0){//if depence count 

       for(const auto& subdepence :  table_.at(front)->GetReferencedCells()) {//for subdepence 
         if(storage.count(subdepence) == 0 ){//if storage count 
             queue_to_visit.push_back(subdepence); 
            if(subdepence == pos) return true; 
       }//if storage count 
       }//for subdepence 
       }//if depence count 
 ...
 void Sheet::PrintDiagnostic() const{
     using namespace std;
     for(auto&& i : table_){
       std::cout<<endl<<"Position("<<i.first.row<<"->"<<i.first.col<<") "<<i.first.ToString() <<endl;
       i.second->PrintDiagnostic();}
       cout<<endl;
       cout<<"--------------------------------------------------"<<endl;
    }